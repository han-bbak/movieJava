    ...
    <section class="contentSection">
        ...
        <div class="contentWrap">
            ...
            <div class="pagingBtnDiv">
            	<!-- 처음으로 -->
           		<button onclick="location.href='<%= request.getContextPath() %>/manager/qaWait?currentPage=1'"> &lt;&lt; </button>
            	
            	<!-- 이전으로 -->
            	<% if(pi.getCurrentPage() == 1) { %>
            		<button disabled> &lt; </button>
            	<% } else { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/qaWait?currentPage=<%= pi.getCurrentPage() - 1 %>'"> &lt; </button>
            	<% } %>
            	...
            	<% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++) { %>
            		...
            		<% } else { %>
            			<button onclick="location.href='<%= request.getContextPath() %>/manager/qaWait?currentPage=<%= p %>'"><%= p %></button>
            		<% } %>
            	<% } %>
            	...
            	<!-- 맨 끝으로 -->
           		<button onclick="location.href='<%= request.getContextPath() %>/manager/qaWait?currentPage=<%= pi.getMaxPage() %>'"> &gt;&gt; </button>
            </div>
        </div>
    </section>
    <script>
