...
<body>
    ...
    <section class="contentSection">
        ...
        <div class="contentWrap">
            ...
            <hr>
            <form method="get" action="<%= request.getContextPath() %>/manager/searchNetflix">
	            <div class="searchDiv">
            ...
            <div class="pagingBtnDiv">
            	<!-- 처음으로 -->
            	<% if(s == null) { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/shareNetflix?currentPage=1'"> &lt;&lt; </button>
            	<% } else { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/searchNetflix?currentPage=1&search=<%= search %>'"> &lt;&lt; </button>
            	<% } %>
            	<!-- 이전으로 -->
            	<% if(pi.getCurrentPage() == 1) { %>
            		<button disabled> &lt; </button>
            	<% } else if(s == null) { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/shareNetflix?currentPage=<%= pi.getCurrentPage() - 1 %>'"> &lt; </button>
            	<% } else {%>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/searchNetflix?currentPage=<%= pi.getCurrentPage() - 1 %>&search=<%= search %>'"> &lt; </button>
            	<% } %>
            	...
            	<% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++) { %>
            		...
            		<% } else if(s == null) { %>
            			<button onclick="location.href='<%= request.getContextPath() %>/manager/shareNetflix?currentPage=<%= p %>'"><%= p %></button>
            		<% } else { %>
            			<button onclick="location.href='<%= request.getContextPath() %>/manager/searchNetflix?currentPage=<%= p %>&search=<%= search %>'"><%= p %></button>
            		<% } %>
            	<% } %>
            	...
            	<!-- 맨 끝으로 -->
            	<% if(s == null) { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/shareNetflix?currentPage=<%= pi.getMaxPage() %>'"> &gt;&gt; </button>
            	<% } else { %>
            		<button onclick="location.href='<%= request.getContextPath() %>/manager/searchNetflix?currentPage=<%= pi.getMaxPage() %>&search=<%= search %>'"> &gt;&gt; </button>
            	<% } %>
            </div>
        </div>
    </section>
